...
void static SecureTagMinter(const CChainParams& chainparams, CConnman& connman,
                         CWallet* pwallet, bool fProofOfStake)
{
    LogPrintf("SecureTagMinter -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("securetag-miner");
    ...
    while (true) {
        try {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SecureTagMinter -- Failed to find a coinstake\n");
                MilliSleep(5000);
                continue;
            }
            auto pblock = std::make_shared<CBlock>(pblocktemplate->block);
            IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);
            LogPrintf("SecureTagMinter -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            if (fProofOfStake)
            {
                ...
                if (!signer.SignBlock()) {
                    LogPrintf("SecureTagMinter(): Signing new block failed \n");
        ...
        {
            LogPrintf("SecureTagMinter -- terminated\n");
            throw;
        }
        catch (const std::runtime_error &e)
        {
            LogPrintf("SecureTagMinter -- runtime error: %s\n", e.what());
//            return;
        }
    }
}
                      ...
                      CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SecureTagMinter, boost::cref(chainparams), boost::ref(connman), nullptr, false));
}
void ThreadStakeMinter(const CChainParams &chainparams, CConnman &connman, CWallet *pwallet)
{
    ...
    try {
        SecureTagMinter(chainparams, connman, pwallet, true);
        boost::this_thread::interruption_point();
