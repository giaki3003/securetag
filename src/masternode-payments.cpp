...
{
    if(!fundamentalnodeSync.IsSynced() || !masternodeSync.IsSynced() || fLiteMode) {
    ...
    if(fnpayments.IsTransactionValid(txNew, nBlockHeight)) {
        LogPrint("fnpayments", "IsBlockPayeeValid -- Valid fundamentalnode payment at height %d: %s", nBlockHeight, txNew->ToString());
        return true;
    }

    if(sporkManager.IsSporkActive(SPORK_9_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT)) {
        LogPrintf("IsBlockPayeeValid -- ERROR: Invalid fundamentalnode payment detected at height %d: %s", nBlockHeight, txNew->ToString());
        return false;
    }

    LogPrintf("IsBlockPayeeValid -- WARNING: Masternode / Fundamentalnode payment enforcement is disabled, accepting any payee\n");
    return true;
}

void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, CTxOut& txoutFundamentalnodeRet, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    // FILL BLOCK PAYEE WITH MASTERNODE/FUNDAMENTALNODE PAYMENT OTHERWISE
    mnpayments.FillBlockPayee(txNew, nBlockHeight, blockReward, txoutMasternodeRet, txoutFundamentalnodeRet);
    LogPrint("mnpayments", "FillBlockPayments -- nBlockHeight %d blockReward %lld txoutMasternodeRet %s txoutFundamentalnodeRet %s txNew %s",
                            nBlockHeight, blockReward, txoutMasternodeRet.ToString(), txoutFundamentalnodeRet.ToString(), txNew.ToString());
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, CTxOut& txoutFundamentalnodeRet) const
{
    ...
    if(!CFundamentalnodePayments().GetBlockPayeeFN(nBlockHeight, payeeFN)) {
    ...
    ExtractDestination(payeeFN, address3);
    CBitcoinAddress address4(address3);

    LogPrintf("CMasternodePayments::FillBlockPayee -- Fundamentalnode payment %lld to %s\n", fundamentalnodePayment, address4.ToString());
}

int CMasternodePayments::GetMinMasternodePaymentsProto() const {
    return sporkManager.IsSporkActive(SPORK_11_MASTERNODE_PAY_UPDATED_NODES)
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SECURETAG\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
